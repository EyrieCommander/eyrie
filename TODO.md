...
- [ ] **Sidebar still shows uninstalled frameworks.** After uninstalling PicoClaw, it persists in the sidebar because discovery still has cached agent entries. The sidebar reads from discovered agents, not registry install status. Should clear on the next discovery cycle, but if stale instances exist in the instance store they keep appearing. Related to the "sidebar should show all installed frameworks" item — both need the sidebar to read from the registry rather than discovery.
